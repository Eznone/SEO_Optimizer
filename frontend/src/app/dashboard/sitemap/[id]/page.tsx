...
export default function SitemapDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  if (loading) return <div className="animate-pulse space-y-4 pt-10 text-center text-gray-400">Loading sitemap details...</div>;
  ...
  return (
    <div className="space-y-8 pb-20">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <nav className="flex items-center gap-2 text-sm text-gray-500 mb-2">
            <Link href="/dashboard/sitemap" className="hover:text-indigo-600">Sitemaps</Link>
            <span>/</span>
            <span className="font-medium text-gray-900 truncate max-w-[200px]">{job.target_url}</span>
          </nav>
          <h1 className="text-3xl font-extrabold text-gray-900">Sitemap Results</h1>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
          <p className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-1">Total Pages</p>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-gray-100 flex items-center justify-between">
          <h2 className="text-xl font-bold text-gray-900">Sitemap Preview</h2>
          <span className="text-sm text-gray-500">Showing XML content link</span>
        </div>
        <div className="p-10 text-center">
          {job.sitemap_url ? (
            <div className="space-y-4">
              ...
              <p className="text-gray-600 font-medium">Your sitemap is ready and hosted at:</p>
              <code className="block bg-gray-50 p-4 rounded-xl border border-gray-200 text-indigo-600 break-all select-all">
                {getBackendUrl(job.sitemap_url)}
              </code>
              <p className="text-xs text-gray-400">Search engines can now discover your pages more effectively.</p>
            </div>
          ) : (
            <div className="space-y-4">
              ...
              <p className="text-gray-600 font-medium">Sitemap is being generated or was not created.</p>
              <p className="text-sm text-gray-400">If the crawl is still in progress, please check back in a few minutes.</p>
            </div>
          )}
        </div>
      </div>
